...
服务器可以使用这个头部明确列出哪些源提供服务，或者使用通配符表示可以接收任何网站的请求。
...
客户端JS原生的API没有提供额外的cookieAPI，想要访问和修改cookie，就必须解析和修改这个`document.cookie`字符串
...
这意味着并发修改DOM仍然是不可能的，但也意味我们可以写长时间运行的函数，而不会阻塞事件循环，卡死浏览器。

创建新工作线程会占用一部分资源，为了执行简单的操作而创建新工作线程是完全没必要的。工作线程适合执行计算密集型任务，比如图像处理。而工作线程也提供了把任务分给多个线程的可能。

和任何线程API一样，WorkerAPI分为两部分。一部分是Worker对象，另一部分是WorkerGlobalScope。前者是线程的外在部分，后者是线程的内在部分。

## Worker对象

要创建新的工作线程，可以调用`Worker()`构造函数，传入一个URL，这个URL用于指定线程要执行的JS脚本代码：

~~~js
let dataCruncher = new Worker("utils/cruncher.js")
~~~

如果传入的是相对URL，则会按照调用`Worker()`构造函数的脚本所在的文档的位置进行解析。如果传入的是绝对URL，则必须和所在文档同源。

创建Worker对象后，可以使用`postMessage()`方法向工作线程发送，工作线程会通过消息事件接收消息。

通过监听Worker对象的`message`事件，可以从工作线程接收消息。和所有事件目标一样，Worker对象定义了标准的`addEventListener()`和`removeEventListener()`方法，可以用它来替代onmessage

除了`postMessage()`，Worker对象只有另一个方法`terminate()`用于强制停止工作线程。

## 工作线程中的全局对象

通过`Worker()`构造函数创建工作线程时，传入的URL指定的时一个JS代码文件。其中的代码会在一个新的、干净的JS执行环境中执行，与创建工作线程的脚本完全隔离。这个新执行环境中的全局对象是一个`WorkerGlobalScope`对象。这个对象比核心JS全局对象多一些东西，但比客户端中完整的Window对象少一些东西。

`WorkerGlobalScope`对象也有`postMessage()`方法和`onmessage`事件处理，只是方向与Worker对象上的相反。

在工作线程内部调用`postMessage()`会在主线程的`Worker()`对象上生成一个消息事件。`WorkerGlobalScope`的onmessage事件监听器在主线程向工作线程发送消息时触发。

因为`WorkerGlobalScope`对象是工作线程的全局对象，`postMessage()`和`onmessage`在工作线程的代码中就像是一个全局函数和全局白能量。

`Worker()`构造函数有可选的第二个对象参数，这个对象有一个name属性，这个属性的值会称为工作线程中全局对象的name属性。工作线程中通过`console.warn()`或者`console.error()`打印的任何消息中，工作线程都会包含这个name

`close()`函数可以让工作线程终结自己，效果和调用主线程`Worker`对象的`terminate()`方法一样

`WorkerGlobalScope`是工作线程的全局对象，所以拥有核心JS全局对象的所有属性，如JSON对象，`isNaN()`、`Date()`函数，除此之外，它还拥有下列客户端Window对象的属性

* self，对自己的引用
* `setTimeout()`、`clearTimeout()`、`setInterval()`、`clearInterval()`等定时器方法
* `location`属性描述传给`Worker()`构造函数的URL，引用一个Location对象。
* navigator属性引用一个类似Window对象的Navigator对象
* 事件目标方法`addEventListener()`和`removeEventListener()`

最后`WorkerGlobalScope`对象还包含重要的客户端JS API，如Console对象、`fetch()`函数和IndexDB API.甚至有`Worker()`构造函数，这意味着工作线程也可以创建自己的工作线程

## 在工作线程中导入代码

浏览器支持Worker的时候JS还不支持模块系统，所以工作线程有自己一套独特的系统用于导入外部代码。WorkerGlobalScope定义了`importScripts()`全局函数，所有工作线程都可以使用：

~~~js
importScripts("xx.js","xxx/xxxxx.js")
~~~

这个函数接收一个或多个URL参数，每个URL引用一个JS代码文件。相对URL的解析相对于传给`Worker()`构造函数的URL(而不是相对于包含文档).`importScripts()`按照传入的顺序依次同步加载并执行这些文件，如果加载某个脚本时出现网络错误，或者脚本本身抛出错误，则后续脚本都不会再加载或执行。

通过`importScritps()`加载的脚本本身也可以调用`importScripts()`加载自己的依赖文件，但是这个函数不会跟踪已经下载了哪些脚本，也不会阻止循环依赖。

## 工作线程执行模型

工作线程自上而下地同步运行自己的代码(和所有导入的脚本和模块)，之后就进入了异步阶段，准备对事件和定时器做出响应。如果注册了`message`监听器，只要有收到消息事件的可能，工作线程就不会退出。

如果工作线程没有监听器，它会运行到没有其他待解决的任务，且所有任务相关的回调都被调用。在所有注册的回调都被调用哦个后，工作线程就不可能再启动新任务了，此时线程会安全自动的退出。

工作线程也可以调用全局的`close()`函数显式地自己终止。但是，Worker对象上没有任何属性和方法可以告诉我们工作线程是否还在运行，所以除非和父线程协商一致，否则工作线程不应该主动终止自己。

### 工作线程中的错误

如果工作线程中出现了异常，并且没有被catch子句捕获，那么它的传播顺序如下：

* 在工作线程的全局对象上触发error事件。如果这个事件有处理程序，并且该处理程序调用了事件对象的`preventDefault()`则错误会停止传播
* 否则，会触发Worker对象上的error事件，如果这里调用了`proeventDefault()`，则传播停止
* 否则，开发者控制台会打印错误信息，并调用主线程`Window`对象的`onerror`处理程序

工作线程也可以使用期约，所以可以在主线程类似，工作线程可以注册一个`unhandledrejection`注册一个全局对象处理程序。

## MessageChannel

Worker对象的`postMessage()`方法和工作线程内部的全局`postMessage()`函数，都是通过调用在创建工作线程时一起创建的一对`MeesagePort`对象的`postMessage()`方法来实现通信的。

客户端JS无法访问这两个自动创建的MessagePort对象，但可以通过`MessageChannel()`构造函数创建一对新的关联端口：
...
一个`MessageChannel`对象有两个属性`port1`和`port2`，引用一对关联的`MessagePort`对象。调用一个`MessagePort`对象上的`postMessage()`方法会触发另一个对象的message事件。

发到到一个消息端口MessagePort的消息在该端口定义`onmessage`或者调用`start()`方法之前会被放在一个队列中。这样可以防止消息被错过。如果时通过`MessagePort`的`addEventListener()`注册的消息监听器，不要忘了调用`start()`方法，否则监听器永远不会触发。

`postMessage()`方法接收可选的第二个参数，这个参数是数组，数组的元素不是被复制到信道的另一端，而是被转移到信道的另一端。

这个数组的元素类型必须是：`ArrayBuffer | MessagePort | ImageBitmap`这三种。

如果`postMessage()`的第一个参数包含一个`MessagePort`(嵌套在消息对象的某个地方),那么该MessagePort也必须出现在第二个参数中。这样依赖，这个MessagePort将被转移到另一个线程，并在当前线程立即失效。

如果已经创建了一个工作线程，但是希望有两个信道用于通信：一个信道用于交换普通数据，另一个信道用于交换高优先级消息。那么可以在主线程中创建一个`MessageChannel`，然后调用`Worker`对象的`postMessage()`方法，把其中一个MessagePort传给工作线程：

~~~js
let worker = new Worker("./worker-thread-demo.js");
...
urgentPort.addEventListener("message",(e)=> console.log("Master Thread Urgent Message:"+e.data))
urgentPort.start();
worker.onmessage = e =>{
    console.log("Master Thread Normal Message:" + e.data)
}

urgentPort.postMessage("A urgent message from master thread");
worker.postMessage("a normal message from master thread")
~~~

对应的，要在工作线程的代码中接收这个urgentChannel.port2：

~~~js
onmessage = function (e) {
    let data = e.data;
    if (data.command === "setUrgentPort"){
        let urgentClientPort = data.value;
        urgentClientPort.onmessage = function (e) {
            console.log("Worker Thread Urgent Message:" + e.data)
        }
    }

    else
        console.log("Worker Thread Normal Message:" + data)
}
~~~

使用MessageChannel也可以实现两个工作线程间的直接通信，从而避免了通过主线程代发消息。

`postMessage()`的第二个参数还可以在工作线程间转移而非复制ArrayBuffer。对于较大的ArrayBuffer，可以在很大程度上提升性能。

如果`postMessage()`的第一个参数中包含一个`ArrayBuffer`，则该ArrayBuffer可以作为数组元素出现在`postMessage()`的第二个参数中。如果出现了，那么它会被转移而非复制。如果没有出现，那么这个ArrayBuffer就会被复制而不会被转移。

## 通过`postMessage()`跨源发送消息

Window对象上也有`postMessage()`方法，这个方法和`MessagePort`上的方法类似但不同。

如果文档中包含一个`<ifram>`元素，则该元素就像一个嵌入但独立的窗口。表示`<iframe>`的Element对象有一个`contentWindow`属性，引用这个嵌套的子文档的Window对象。对于这个子窗口中运行的脚本，`window.parent`属性引用父文档的Window对象。

当两个窗口同源时，两个窗口中的脚本都拥有访问另一个窗口中内容的权限。但如果两个窗口不同源，浏览器的同源策略会阻止两个窗口中的JS相互访问对方的内容。

而窗口Window的`postMessage`函数为两个独立来源提供了安全交换消息的受控机制。在子窗口中可以调用父窗口的`postMessage`方法，这样会触发父窗口的`message`事件，从而让该窗口脚本中的事件监听器接收到。

Window对象上的`postMessage()`方法和工作线程的`postMessage()`方法有一点不同。第一个参数仍然是任意消息对象。但包含要转移而非复制的第二个可选参数变成了可选的第三个参数。window的`postMessage()`方法以一个字符串作为其必须的第二个参数。这第二个参数应该是一个源(协议、主机名和可选的端口号)，用于指定你希望谁接收这条消息。如果传入`https://good.example.com`作为第二个参数，那么如果把消息发送到了源为`https://malware.exmaple.com`的窗口，那么发送的消息就不会被派发。如果想要把消息发送给任意来源的窗口，可以传通配符`*`作为第二个参数
