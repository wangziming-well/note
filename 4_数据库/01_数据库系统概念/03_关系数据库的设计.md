...
以关系模式$R<U,F>,U=\{A,B,C,D,E\},F=\{A\to BC,ABD\to CE,E\to D \}$为例，我们来进行3NF分解
...
首先通过保证至少有一个模式包含被分解模式的候选码，确保3NF分解是一个无损分解。

接下来证明其分解获得的模式属于3NF:我们用定义证明，我们只需要证明函数依赖右半部分是单个属性的情况就可以了，所以只需要证明分解获得的模式$R_i$上的任意函数依赖$\gamma \to B$满足3NF的定义即可。

假设产生$R_i$的函数依赖是$\alpha\to\beta$。因为$B$属于$R_i$,而$\alpha\to\beta$产生了$R_i$，所以$B$一定属于$\alpha$或者$\beta$。所以只有下面三种情况:

* $B$即属于$\alpha$也属于$\beta$,这种情况下$B$为$\beta$的无关属性，这于$\beta \in F_c$矛盾，所以这种情况不存在
* $B$只属于$\beta$:
  * 若$\gamma$是一个超码，那么满足3NF的条件
  * 若$\gamma$不是一个超码，则$\alpha \cap (R_i - \gamma) $非空，(否则)

* $B$只属于$\alpha$:因为$\alpha$是候选码，满足了3NF的第三个条件。
